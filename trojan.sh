...
getVersion() {
    VERSION=`curl -fsSL ${V6_PROXY}https://api.github.com/repos/gfw-report/trojan-go/releases | grep tag_name | sed -E 's/.*"v(.*)".*/\1/'| head -n1`
...
getCert() {
    ...
    if [[ -z ${CERT_FILE+x} ]]; then
        ...
        curl -sL https://get.acme.sh | sh -s email=waiquwang@gmail.com
        source ~/.bashrc
        ~/.acme.sh/acme.sh  --upgrade  --auto-upgrade
	~/.acme.sh/acme.sh --register-account -m waiquwang@gmail.com --server zerossl
...
downloadFile() {
    SUFFIX=`archAffix`
    DOWNLOAD_URL="${V6_PROXY}https://github.com/gfw-report/trojan-go/releases/download/${VERSION}/trojan-go-linux-${SUFFIX}.zip"
